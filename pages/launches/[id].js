...
export default function () {
  ...
  return (
    <div className='flex items-center min-h-screen flex-col p-8'>
            ...
            
            {
              launch.rocket.rocket && (
                <>
                  ...
                  <span className='text-center text-sm'>{ launch.rocket.rocket.boosters } Boosters</span>
                  <span className='text-center text-sm'>{ launch.rocket.rocket.mass.kg } kg</span>
                </>
              )
            }
          </div>
      ...
      <button
        className={`p-4 bg-white border-2 border-violet-800 mt-8 rounded-xl shadow-lg flex items-center justify-center mt-1 h-16 w-36 hover:border-black${loading ? ' cursor-not-allowed' : ' cursor-pointer active:bg-slate-200'}`}
        disabled={loading}
        onClick={() => router.back()}
        type='button'
      >
